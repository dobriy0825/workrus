...
@section('content')
    <section class="" >
        <form id="create_job_form" action="{{ route('job.store') }}" method="post">
            @method('post')
            @csrf
        </form>


        <h2 class="section__title">Создание задания</h2>
        @if ($errors->any())
        ...
        <article class=" workerStyle size_article700 bgc__createWorker__pc">
            <div class="block">
                ...
                <input class="workerStyle__inputText" type="text" form="create_job_form" name="title"
                       placeholder="Введите название задания" value="{{ old('title') }}" style="padding: 7px 7px;">
            </div>
            <div class="block">
...
{{--                <input class="worker_category__input" type="text" name="category" form="create_job_form">--}}
                ...
                <select name="category" form="create_job_form" class="workerStyle__inputSelect select_category"
                        style="padding: 7px 5px;">
                    <option selected value="">Категория</option>
                    @foreach($categories as $category)
                        <option value="{{ $category->name }}" {{ $category->name == old('category') ? 'selected' : '' }}>{{ $category->name }}</option>
                    @endforeach
                </select>
                <select name="sub_category" form="create_job_form" style="margin-left: 11px;margin-bottom: 10px; width: 251px;padding: 7px 5px;" class="workerStyle__inputSelect select_sub_category">
                    <option class="name_option" selected value="{{ old('sub_category') }}">{{ old('sub_category') ? old('sub_category') : 'Подкатегория' }}</option>
                </select>
            ...
            <div class="block">
                ...
                <hr class="workerStyle__line">
                <span class="btn_for_selected btn_show_towns" style="cursor: pointer;">Выбрать город</span>
            ...
            <div class="block">
                ...
                <div  class="workerStyle__sex_items">
                    <div class="workerStyle__sex_item">
                        <input class="workerStyle__inputRadio" type="radio" form="create_job_form" name="need_people" value="1" id="radio1" checked>
                        <label class="workerStyle__label" for="radio1">1</label>
                    </div>

                    <div  class="workerStyle__sex_item">
                        <input class="workerStyle__inputRadio" type="radio" form="create_job_form" name="need_people" value="2" id="radio2">
                        <label class="workerStyle__label" for="radio2">2</label>
                    </div>
                    <div  class="workerStyle__sex_item">
                        <input class="workerStyle__inputRadio" type="radio" form="create_job_form" name="need_people" value="3" id="radio3">
                        ...
                        <div class="wrap_date_time">
                            <div class="wrap_icon_input wrap_date">
                                ...
                                <input class="date_time__datepicker" type="text" placeholder="Дата"
                                       form="create_job_form" name="date_start" value="{{ old('date_start') }}">
                            </div>
...
{{--                                       name="time1" value=":">--}}
                                <select name="time_start" form="create_job_form">
                                    @foreach($time as $item)
                                        <option value="{{ $item }}" {{ $item == old('time_start') ? 'selected' : '' }}>{{ $item }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="date_time date_time_finish">
                        ...
                        <div class="wrap_date_time">
                            <div class="wrap_icon_input wrap_date">
                                ...
                                <input class="date_time__datepicker" type="text" placeholder="Дата"
                                       form="create_job_form" name="date_finish" value="{{ old('date_finish') }}">
                            </div>
                            <div class="wrap_icon_input wrap_time">
                                <div class="bg_icon">
                                    <i class="date_time__icon fas fa-clock"></i>
...
{{--                                       name="time_finish" value="{{ old('time_finish') }}">--}}
                                <select name="time_finish" form="create_job_form">
                                    @foreach($time as $item)
                                        <option value="{{ $item }}" {{ $item == old('time_finish') ? 'selected' : '' }}>{{ $item }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
